...
import {DayToken, isDayToken, TokenType} from "./types";

export function convert(inputString: string): Date {
    let tokens = tokenizer(inputString);
    let date = new Date();
    if (tokens[0] && tokens[0].type === TokenType.day) {
        let firstToken = tokens.shift() as DayToken;
